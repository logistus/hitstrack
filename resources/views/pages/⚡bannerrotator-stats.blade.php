...
<section class="container mx-auto space-y-8" x-data="{ activeTab: $wire.entangle('activeTab') }" data-tracker-stats-root>
    ...
    <div class="space-y-6">
        <div class="inline-flex rounded-lg border border-zinc-200 bg-white p-1 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <button type="button" wire:click="showTab('events')" x-on:click="$wire.showTab('referrers').then(() => activeTab = 'referrers')" class="rounded-md px-3 py-1.5 text-sm font-medium transition" :class="activeTab === 'events' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white'">{{ __('Daily events') }}</button>
            <button type="button" wire:click="showTab('banners')" x-on:click="$wire.showTab('referrers').then(() => activeTab = 'referrers')" class="rounded-md px-3 py-1.5 text-sm font-medium transition" :class="activeTab === 'banners' ? 'bg-zinc-900 text-white dark:bg-white dark:text-zinc-900' : 'text-zinc-600 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white'">{{ __('Banners') }}</button>
